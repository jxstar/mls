...
if(T){
  ...
  glastsku=dbGetQuery(conn,"select gid, date, code, nid, sid, sales, likes, price 
                      from (select * from simple_mls_sku where date<=20150710 order by date desc) as sku group by gid order by date desc")
  glasthd=dbGetQuery(conn,"select sid,date,grade, goods, sales, likes
                      from (select * from simple_mls_haodian where date<=20150710 order by date desc) as hd group by sid order by date desc")
  ...
  names(glastsku)=c("gid", "date", "tid2", "tid3", "sid", "sales", "likes", "price")
  glastsku=select(glastsku,date,gid,tid2,tid3,sid,sales,likes,price)
  glastsku$date=as.Date(as.character(glastsku$date),format="%Y%m%d")
  names(glasthd)=c("sid", "date", "grade", "ngoods", "sales", "likes")
  glasthd=select(glasthd,date,sid,grade, ngoods, sales,likes)
  glasthd$date=as.Date(as.character(glasthd$date),format="%Y%m%d")
  glaststore=distinct(select(glastsku,sid),sid)
  dim(glaststore)
...
if (F){
  ...
  dbGetQuery(conn,"set names gbk") # for win
  #12 19 26
  category=dbGetQuery(conn,"select * from simple_mls_cat where date>20150712 and date<=(20150811)")
  haodian=dbGetQuery(conn,"select * from simple_mls_haodian where date>20150712 and date<=(20150811)")
  sku=dbGetQuery(conn,"select * from simple_mls_sku where date>20150712 and date<=(20150811)")
  dbDisconnect(conn)
...
#load(file="glastsku.Rdata")
glastsku=glaststatus
...
(gfile=paste(previous,"to",max(gdate),"MLS Analysis Result By JX @", Sys.Date(),".xlsx",sep = " "))
...
dealist$mfactor=apply(dealist[,c("lastdate","maxdate")],1,function(x){
  if (!is.na(x[1])) return((x[2]-as.integer(previous))/(x[2]-x[1]))
  else return(1)
})
...
dealist[,"ndeals"]= apply(dealist[,c(gcolx,"m0","mfactor")],1, 
                          function(x){return((max(x[1:gnperiod],na.rm=T)-sum(x[gnperiod+1],na.rm=T))*x[gnperiod+2])})
...
dealist$GMV=apply(dealist[,c("m0",gcolx,gcoly)],1,function(x){
  GMV=c()
  for (i in 1:gnperiod){
    GMV[i]=(sum(x[i+1],na.rm=T)-sum(x[i],na.rm=T))*x[i+5]
  }
  return(sum(GMV,na.rm=T)/100)  
})
...
hdlike$nlikes=0
#hdlike[,"nlikes"]= apply(hdlike[,c(gcol,"m0","mfactor")],1, 
#                        function(x){return((-sum(x[gnperiod+1],na.rm=T))*x[gnperiod+2])})

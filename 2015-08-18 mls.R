...
write.csv(file=paste(last(previous)+1,"to",max(gdate),"MLS store list", Sys.Date(),".csv",sep = " "),
x=left_join(storecur,distinct(select(sku,sid,sname),sid),by=c("sid"="sid")), na = "")
...
#write.csv(file=paste(last(previous)+1,"to",max(gdate),"MLS haodian list", Sys.Date(),".csv",sep = " "),
#          x=left_join(hdb,distinct(select(haodian,sid,sname),sid),by=c("sid"="sid")), na = "")
write.csv(file=paste(last(previous)+1,"to",max(gdate),"MLS haodian list", Sys.Date(),".csv",sep = " "),
          x=distinct(arrange(haodian,desc(date)),sid), na = "")

write.csv(file=paste(last(previous)+1,"to",max(gdate),"MLS SKU list", Sys.Date(),".csv",sep = " "),
          x=distinct(arrange(sku,desc(date)),gid), na = "")
